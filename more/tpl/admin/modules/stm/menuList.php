...
href="<?= Sflm::get('css')->sflm->getCachedUrl('s2/css/common/horizontalMenu.css?ids=menu') ?>" media="screen, projection" />
*/?>
...
foreach ($d['DbItemsExtended'] as $v) {
  ...
  <style>
  <?= str_replace('.hMenu', '.hMenu.custom'.$i, $o->css->css) ?>
  </style>
  
  <div class="item">
    <? if ($v->canEdit) { ?> 
      <a href="<?= $this->getPath(2).'/editMenu/'.$v->source->options['location'].'/'.
        $v->source->options['menuType'].'/'.$v->n ?>"
        class="iconBtn edit" title="Редактировать меню"><i></i></a>
      <a href="<?= $this->getPath(2).'/deleteMenu/'.$v->source->options['location'].'/'.
        $v->source->options['menuType'].'/'.$v->n ?>"
        class="iconBtn delete confirm" title="Удалить меню"><i></i></a>
    <? } else { ?>
      <a href="" class="iconBtn dummy"><i></i></a>
    <? } ?>
    <h2><?= $v->source->structure['title'].' / <b>'.$v->data['title'].'</b>' ?></h2>
