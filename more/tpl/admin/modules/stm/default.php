...
foreach ($d['items'] as $v) {
  $link = '/?theme[location]='.$v->source->options['location'].'&theme[design]='.
    ...
    $a = $v->source->options['location'].'/'.
         $v->source->options['siteSet'].'/'.
         $v->source->options['design'].'/'.
         $v->n;
  ?>
  <li class="item<?= $v->selected ? ' selected' : '' ?>">
    <? if (!$v->selected) { ?>
    <a href="<?= $this->getPath(2).'/setTheme/'.$a ?>" class="btn"><span>Установить</span></a>
    <? } ?>
    <div class="tools">
      <a href="<?= $link ?>" target="_blank" 
        title="Предварительный просмотр" class="iconBtn preview"><i></i></a>
      <? if ($v->canEdit) { ?>
        <a href="<?= $this->getPath(2).'/editTheme/'.$v->source->options['location'].'/'.
          $v->source->options['siteSet'].'/'.$v->source->options['design'].'/'.
          $v->n
          ?>" title="Настройки темы" 
          class="iconBtn settings themeSettings"><i></i></a>
        <a href="<?= $this->getPath(2).'/deleteTheme/'.$v->source->options['location'].'/'.
          $v->source->options['siteSet'].'/'.$v->source->options['design'].'/'.
          $v->n
          ?>" title="Удалить тему" 
    ...
    <div class="gray designTitle"><small>Дизайн: <?= $v->source->structure['title'] ?></small></div>
    <div class="clear"><!-- --></div>
    <div class="body">
