...
define("SMSC_LOGIN", Config::getVarVar('smsc', 'login', true));    // логин клиента
define("SMSC_PASSWORD", Config::getVarVar('smsc', 'pass', true));  // пароль или MD5-хеш пароля в нижнем регистре
...
class Smsc {
  ...
  /**
   * Вызов запроса к API. Формирует URL и делает 3 попытки чтения
   *
   * @param $cmd
   * @param string $arg
   * @param array $files
  ...
  protected function smscSendCmd($cmd, $arg = "", $files = array()) {
    ...
    if ($ret == "") {
      if (SMSC_DEBUG) echo "Ошибка чтения адреса: $url\n";
      $ret = ","; // фиктивный ответ
    }
    $delim = ",";
    if ($cmd == "status") {
  ...
  function _smsc_read_url($url, $files) {
    ...
    if (function_exists("curl_init")) {
      ...
      if ($post) {
        ...
        if ($files) {
          ...
          $post = $m;
          foreach ($files as $i => $path) if (file_exists($path)) $post["file".$i] = function_exists("curl_file_create") ? curl_file_create($path) : "@".$path;
        }
    ...
    elseif ($files) {
      throw new SmscException("Не установлен модуль curl для передачи файлов");
    }
    else {
      if (!SMSC_HTTPS && function_exists("fsockopen")) {
        $m = parse_url($url);
        if (!$fp = fsockopen($m["host"], 80, $errno, $errstr, 10)) $fp = fsockopen("212.24.33.196", 80, $errno, $errstr, 10);
        if ($fp) {
