...
class Pagination extends Options2 {
  ...
  function get($table, $cond = '', $selectCond = '') {
    ...
    $res = $this->data($cnt);
    $r = [$res[0], $res[1].','.($res[2]), $res[3], $res[4], $res[5], $res[6]];
    return $r;
  }
  ...
  function data($all) {
    ...
    if ($pagesN != 0 and $pagesN != 1) {
      ...
      for ($i = 0; $i < $pagesN; $i++) {
        ...
        if ($page != $pagesN) $pNext = $this->options['selfPage'].$self.'/pg'.$this->options['type'].($page + 1);
        if ($page - 1 != 0) $pPrev = $this->options['selfPage'].$self.'/pg'.$this->options['type'].($page - 1);
      }
    }
    ...
    return [$html, $offset, $limit, $all, count($links), $pNext, $pPrev];
  }
