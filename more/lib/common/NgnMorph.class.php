...
class NgnMorph {
   ...
   * @return phpMorphy
   */
  static function getPhpMorphy() {
    require_once VENDORS_PATH.'/phpmorphy/src/common.php';
    if (isset(self::$o)) return self::$o;
    self::$o = new phpMorphy(VENDORS_PATH.'/phpmorphy/dicts', 'ru_RU');
    return self::$o;
  }

  static function _singular2plural($word) {
    return self::cast($word, ['МН', 'ИМ']);
  }
   ...
   */
  static function _cast($word, $options = ['ЕД', 'ИМ']) {
    $r = self::getPhpMorphy()->castFormByGramInfo(mb_strtoupper($word, CHARSET), null, $options, true);
    return mb_strtolower($r[0], CHARSET);
  }

  static function _gender($word, array $genders) {
    $info = self::getPhpMorphy()->getGramInfo(mb_strtoupper($word, CHARSET));
    if ($info[0][0]['grammems'][2] == 'МР') return $genders[0];
    if ($info[0][0]['grammems'][2] == 'ЖР') return $genders[1];
    if ($info[0][0]['grammems'][2] == 'СР') return $genders[2];
  }
  ...
  static function _name($name, $case = 2) {
