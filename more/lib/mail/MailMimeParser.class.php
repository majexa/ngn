...
class MailMimeParser {
  ...
  */
      ...
      case MIME_PARSER_HEADER:
        ...
        if (($break = strspn($this->buffer, $this->header_name_characters, $this->buffer_position) + $this->buffer_position) < strlen($this->buffer)) {
          switch ($this->buffer[$break]) {
            ...
            default:
              if (!$this->SetPositionedWarning('message headers do not end with empty line', $this->buffer_position)) return (0);
              $part = array(
                'Type'     => 'BodyStart',
                'Position' => $this->offset + $this->buffer_position
              );
  ...
  function DecodePart($part) {
    switch ($part['Type']) {
      case 'MessageStart':
        ...
        break;
      case 'HeaderName':
        if ($this->decode_bodies) $this->current_header = strtolower($part['Name']);
        break;
      case 'HeaderValue':
        if ($this->decode_headers) {
          ...
          for ($decoded_header = array(), $position = 0; $position < strlen($value);) {
            ...
            $position = $end + 2;
            if (($space = strspn($value, " \t", $position)) > 0) $position += $space - 1;
          }
          ...
          switch ($this->current_header) {
            ...
            case 'content-transfer-encoding:':
              switch ($this->headers['Encoding'] = strtolower(trim($part['Value']))) {
                ...
                case '7 bit':
                case '8 bit':
                  if (!$this->SetPositionedWarning('"'.$this->headers['Encoding'].'" is an incorrect content transfer encoding type', $part['Position'])) return (0);
                case '7bit':
                case '8bit':
                case 'binary':
                  break;
                case 'base64':
                  ...
                  break;
                default:
                  if (!$this->SetPositionedWarning('decoding '.$this->headers['Encoding'].' encoded bodies is not yet supported', $part['Position'])) return (0);
              }
              break;
          }
        }
              ...
              case MIME_PARSER_BODY_START:
                for ($position = $this->body_buffer_position; ;) {
                  ...
                  if (!strcmp(rtrim(substr($this->body_buffer, $position, $line_break - $position)), $boundary)) {
                    ...
                    $this->body_parser = new MailMimeParser;
          ...
          switch ($type) {
            case 'HeaderValue':
              ...
              elseif (GetType($decoded['Headers'][$header]) == 'string') {
                ...
                if ($this->extract_addresses && IsSet($this->address_headers[$header])) $decoded['HeaderPositions'][$header] = array(
            ...
            case 'BodyData':
              if (IsSet($parameters['SaveBody'])) {
                if (!IsSet($decoded['BodyFile'])) {
                  ...
                  if ($this->use_part_file_names && !$this->GetPartFileName($decoded, $filename)) return (0);
              ...
              continue 3;
            case 'StartPart':
              if (!$this->DecodeStream($parameters, $position + $part['Position'], $end_of_part, $decoded_part)) return (0);
              $decoded['Parts'][$part_number] = $decoded_part;
              $part_number++;
              $state = MIME_MESSAGE_GET_BODY_PART;
              continue 3;
            case 'MessageEnd':
              if (IsSet($decoded['BodyFile'])) fclose($this->body_file);
              return (1);
          }
          break;
  ...
  function GetPartFileName($decoded, &$filename) {
    if (IsSet($decoded['FileName'])) $filename = basename($decoded['FileName']);
    return (1);
  }
        ...
        <documentation>
          <purpose>Retrieve the structure of the parsed message headers and
            ...
            array with the format of the <link>
              <data>addresses</data>
              <url>MailRfc822Addresses.html#argument_ParseAddressList_addresses</url>
            </link> argument of the <link>
              <data>ParseAddressList</data>
              <url>MailRfc822Addresses.html#function_ParseAddressList</url>
            </link> function of the <link>
              <data>RFC 822 addresses</data>
              <url>MailRfc822Addresses.html</url>
  ...
  */
  function decode($parameters) {
    ...
    $addresses = new MailRfc822Addresses;
    ...
    if (!$success) throw new Exception("Error while parsing mail mime file '{$this->file}'");
    return $decoded;
  }
  ...
  */
  ...
  function CopyAddresses($message, &$results, $header) {
    ...
    if (!IsSet($message['ExtractedAddresses'][$header])) {
      $parser = new MailRfc822Addresses;
    ...
    switch ($type) {
      case 'multipart':
        ...
        switch ($sub_type) {
          ...
          case 'signed':
            if ($lp != 2) return ($this->SetError('this '.$content_type.' message does not have just 2 parts'));
          ...
          case 'form-data':
            ...
            for ($p = 0; $p < $lp; ++$p) {
              if (!IsSet($message['Parts'][$p]['Headers']['content-disposition:'])) return ($this->SetError('the form data part '.$p.' is missing the content-disposition header'));
              ...
              if (strcmp($disposition, 'form-data')) {
                if (!$this->SetPositionedWarning('disposition of part '.$p.' is not form-data', $message['Parts'][$p]['Position'])) return (0);
                continue;
              }
      ...
      case 'application':
        switch ($sub_type) {
          ...
          case 'vnd.ms-excel':
            ...
            break;
          case 'x-compressed':
            if (!IsSet($parameters['name']) || GetType($dot = strpos($parameters['name'], '.')) != 'integer' || strcmp($extension = strtolower(substr($parameters['name'], $dot + 1)), 'zip')) break;
          case 'zip':
          case 'x-zip':
