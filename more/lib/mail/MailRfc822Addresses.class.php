...
class MailRfc822Addresses {
  ...
  */
  ...
  Function QDecode($p, &$value, &$encoding) {
    ...
    while ($s < $l) {
      ...
      switch ($type) {
        ...
        case 'b':
          if ($e <= $t || strlen($binary = base64_decode($data = substr($value, $t, $e - $t))) == 0 || GetType($binary) != 'string') return ($this->SetPositionedWarning('invalid Q-encoding b encoded data', $p + $t));
          $decoded .= $binary;
          $s = $e;
          break;
    ...
    for ($space = ''; $p < $l;) {
      switch ($w = $v[$p]) {
        ...
        case "\t":
          ...
          break;
        case '(':
          if (!$this->ParseComment($p, $comment)) return (0);
        default:
          return (1);
      }
    }
  ...
  Function ParseWord(&$p, &$word, &$escape) {
    ...
    for ($w = ''; ;) {
      $last = $w;
      if (!$this->ParseAtom($p, $atom, 0)) return (0);
      if (IsSet($atom)) $w .= $atom;
      if ($this->ignore_syntax_errors) {
        $e = $p;
        if (!$this->ParseQuotedPair($p, $quoted_pair)) return (0);
        if (IsSet($quoted_pair)) {
          $w .= $quoted_pair;
          if (!IsSet($escape)) $escape = $e;
        }
      }
      if ($last === $w) break;
    }
  ...
  Function ParseObsPhrase(&$p, &$obs_phrase, &$escape) {
    ...
    $l = strlen($v);
    $ph = $p;
    if (!$this->ParseWord($ph, $word, $escape)) return (0);
    $string = $word;
    for (; ;) {
      if (!$this->ParseWord($ph, $word, $escape)) return (0);
      if (IsSet($word)) {
  ...
  Function ParsePhrase(&$p, &$phrase, &$escape) {
    $phrase = null;
    if (!$this->ParseObsPhrase($p, $phrase, $escape)) return (0);
  ...
  Function ParseName(&$p, &$address) {
    $address = $escape = null;
    $a = $p;
    if (!$this->ParsePhrase($a, $display_name, $escape)) return (0);
    if (IsSet($display_name)) {
      if (IsSet($escape) && !$this->SetPositionedWarning('it was used an escape character outside a quoted value', $escape)) return (0);
  ...
  Function ParseGroup(&$p, &$address) {
    ...
    $l = strlen($v);
    $g = $p;
    if (!$this->ParsePhrase($g, $display_name, $escape)) return (0);
