...
class TestFilterStr extends ProjectTestCase {
  ...
  function test() {
    ...
    $this->assertNotEmpty(db()->selectCell("select * from tagItems WHERE strName='a' AND groupId='catalog'"));
    $bfm->deleteByName('catalog');
    $this->assertEmpty(db()->selectCell("select * from tagItems WHERE strName='a' AND groupId='catalog'"));
    $sm->delete($id1);
    $sm->delete($id2);
  }
