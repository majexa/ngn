...
class DdTagsHtml {
  ...
  static function treeArrowsLinks($v) {
    return self::treeArrows($v['tags'], '`<a href="'.$v['pagePath'].'/t2.`.$groupName.`.`.$id.`">`.$title.`</a>`');
  }

  static function treeArrows2($v) {
    return Tt()->enumDddd($v['tags'], '`<a href="'.$v['pagePath'].'/t2.`.$groupName.`.`.$id.`">`.$title.`</a>`', ' → ');
  }

  static function treeArrows3($v, $dddd = '`<li>`.$v.`</li>`') {
    $r = '';
    foreach ($v['tags'] as $tags) {
      $html = Tt()->enumDddd([$tags], '`<a href="'.$v['pagePath'].'/t2.`.$groupName.`.`.$id.`">`.$title.`</a>`', ' → ');
