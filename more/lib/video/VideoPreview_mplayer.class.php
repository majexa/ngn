...
class VideoPreview_mplayer {
   ...
   */
  function makePreview($videoId, $videoFilePath, $resultFile, $width = 100, $height = 75,
  $type = VIDEO_PREVIEW_TYPE_FOUR) {
    ...
    if (!isset($this->mplayer))
      throw new Exception('$this->mplayer not defined');
    ...
    if ($type == VIDEO_PREVIEW_TYPE_ONE) {
      ...
      while ($entry = $d->read()) {
        if (is_file($dirPath."/".$entry)) {
          $n++;
          if ($n == VIDEO_PREVIEW_FIRST_STEP) $files = [$entry];
        }
      }
