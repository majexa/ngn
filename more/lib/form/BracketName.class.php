...
class BracketName {
  ...
  static function getKeys($bracketName, $exceptLastKey = false) {
    $m = [];
    ...
    for ($i = 0; $i < count($m[1]) - ($exceptLastKey ? 1 : 0); $i++) $keys .= "['".$m[1][$i]."']";
    return $keys;
  }

  static function getPureName($bracketName) {
  ...
  static function setValue(array &$data, $bracketName, $value) {
    if (($keys = self::getKeys($bracketName)) !== false) {
