...
class Jevix {
  ...
  public $dotes = "…";
  public $nl = "\r\n";
  public $defaultTagParamRules = array('href'   => '#link',
                                       'src'    => '#image',
                                       'width'  => '#int',
                                       'height' => '#int',
                                       'text'   => '#text',
   ...
   * @var array
   */
  protected $chClasses = array(0    => 512,
                               1    => 512,
                               2    => 512,
                               3    => 512,
                               4    => 512,
  ...
  protected function anyThing(&$content = '', $parentTag = null) {
    ...
    while ($this->curChClass) {
      ...
      $shortTag = false;
      $name = null;
      // Если мы находимся в режиме тега без текста, пропускаем контент пока не встретится "<"
      ...
      elseif ($this->curCh == '<') {
        // Если встречается "<", но это не тег, то это либо закрывающийся тег либо знак "<"
        $this->saveState();
        if ($this->tagClose($name)) {
          // Если это закрывающийся тег, то мы делаем откат и выходим из функции.
  ...
  protected function text(&$text) {
    ...
    while (($this->curCh != '<') && $this->curChClass) {
      ...
      elseif ($this->isAutoBrMode && $this->skipNL($brCount)) {
        // Перенос строки
        if ($this->curParentTag and isset($this->tagsRules[$this->curParentTag]) and isset($this->tagsRules[$this->curParentTag][self::TR_TAG_NO_AUTO_BR]) and (is_null($this->openedTag) or isset($this->tagsRules[$this->openedTag][self::TR_TAG_NO_AUTO_BR]))
        ) {
          // пропускаем <br/>
        }
        else {
