...
- `DbModelCore::[b]create[/b]($table, $data, [$filterByFields]);`<br>Создаёт запись с данными `$data` в таблице `$table` и возвращает ID записи. Если `$filterByFields=true`, данные фильтруются по именам полей
- `DbModelCore::[b]createAndGet[/b]($table, $data, [$filterByFields]);`<br>То же самое, что и `create`, только возвращает созданную модель, а не ID
...
<div class="api" markdown="1"><div class="help">@api</div>- `DdStructureCore::[b]create[/b]($name, $fields);`<br>Создаёт структуру
- `DdStructureCore::[b]rename[/b]($name, $newName);`<br>Переименовывает структуру
</div>
...
<div class="api" markdown="1"><div class="help">@api</div>  - `Ddo::[b]setItem[/b]($item);`<br>Задаёт dd-запись и выключает списочный режим вывода
  - `Ddo::[b]setItems[/b]($items);`<br>Задаёт dd-записии и включает списочный режим вывода
  - `Ddo::[b]el[/b]($value, $fieldName, $itemId);`<br>Возвращает HTML элемента dd-записи
  - `Ddo::[b]els[/b]();`<br>Возвращает HTML с элементами всех записей. Каждая запись обрамляется HTML-кодом, генерируемым методами Ddo::itemsBegin и Ddo::itemsEnd
  - `Ddo::[b]table[/b]();`<br>Возвращает HTML-таблицу, где каждая dd-запись - это строка в таблице, а в ячейках находятся отрендереные значения этих записей
  - `Ddo::[b]xls[/b]($file, [$header]);`<br>Созраняет отрендеренные данные в Excel-файл
  - `Ddo::[b]elsSeparate[/b]();`<br>Рендерит каждую запись отдельно и возвращает массив с ними
  - `Ddo::[b]groupFrom[/b]($fieldName);`<br>Обрамляет элементы HTML-контейнером, начиная с указанного поля включительно
</div>
