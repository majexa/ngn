...
class DdTextReplacer {
  ...
  private function processDDItems($type = 'get') {
    if (!$this->from) throw new Exception('$this->from not defined. Use setReplaceText()');
    $this->resultsCount = 0;
    $oS = new DdStructure();
    $oSD = new SearchDescriber([$this->from]);
    $oSD->stripHtml = false;
    $oSD2 = new SearchDescriber([$this->to]);
