...
class CtrlCommonMosaic extends CtrlCommon {
  ...
  function action_createMosaic() {
    if ($this->uploadedImages) {
      $ic = new ImageComposer();
      $ic->mozaicW = $this->req->r['bigW'];
      $ic->mozaicH = $this->req->r['bigH'];
      $ic->_mosaic($this->req->r['w'], $this->req->r['h'], $this->uploadedImages, $this->resultFileAbs);
      $this->redirect();
    } else
