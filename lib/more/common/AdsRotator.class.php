...
class AdsRotator {
	...
	static function html($place, $pageId, $tpl = 'img') {
	...
	static private function log($banner, $table) {
		...
		db()->query("INSERT INTO $table SET ?a", [
      'adsId' => $banner['id'],
      'dateEvent' => dbCurTime(),
  ...
  static function click($id) {
    ...
    $oDdItems = O::get('DdItemsPage', 'ads');
    if ($banner = $oDdItems->getItem($id)) {
      $oDdItems->update($banner['id'], ['clicks' => $banner['clicks']+1]);
