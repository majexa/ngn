...
class Settings {
  ...
  static function get($id, $returnFalseOnEmpty = false) {
    ...
    return empty($r) ? ($returnFalseOnEmpty ? false : []) : unserialize($r);
  }

  static function getItems($prefix) {
    $r = [];
    foreach (db()->select("SELECT id, settings FROM ".static::$table." WHERE id LIKE ?", $prefix.'%') as $v) {
  ...
  static function set($id, $settings) {
    if (!self::get($id)) db()->query("INSERT INTO ".static::$table." SET id=?", $id);
    db()->query("UPDATE ".static::$table." SET settings=? WHERE id=?", serialize($settings), $id);
  }
