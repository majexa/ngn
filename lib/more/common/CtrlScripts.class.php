...
class CtrlScripts extends CtrlCommon {
  ...
  protected function init() {
    ...
    if (strstr($path, 'js/')) {
      ...
      foreach (Sflm::get('js')->pathVariants($path) as $_path) if (Lib::required($_path, $this->req, '')) return;
    }
    elseif (strstr($path, 'css/')) {
      header('Content-type: text/css; charset='.CHARSET);
      foreach (Sflm::get('css')->pathVariants($path) as $_path) if (Lib::required($_path, $this->req, '')) return;
    }
    else {
