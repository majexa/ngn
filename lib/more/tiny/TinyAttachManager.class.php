...
class TinyAttachManager {
  ...
  static function getFolderPath($attachId) {
    return UPLOAD_PATH.'/ed/'.str_replace('-', '/', $attachId); 
  }
  
  static function getFolder($attachId) {
   ...
   */
  static function clearNonUsedAttachFiles($html, $attachId) {
  ...
  static function resizeImages(&$html) {
    /**
    ...
    if ($imageLinks) {
      foreach ($imageLinks as $link) {
        ...
        //  continue;
        //}
        //if (!$sizes) throw new Exception("Error reading '$file'");
        
        if ($sizes[0] > $maxWidth) {
          $heigth = round($maxWidth / ($sizes[0] / $sizes[1]));
          O::get('Image')->resampleAndSave(
            $file,
            Misc::getFilePrefexedPath($file, 'md_'),
            $maxWidth, 
            $heigth
          );
          ...
          $html = preg_replace_callback(
            ...
            create_function(
              ...
              'return
                ...
                str_replace("[mdImage]", Misc::getFilePrefexedPath($m[2], "md_"),
                  \''.self::$thumbTpl.'\'));'
            ),        
            $html
          );
   ...
   */
  static function cleanupImages(&$html, $attachId) {
    set_time_limit_q(0);
  ...
  static function downloadRemoteImages(&$html, $attachId) {
   ...
   * 
   */
  static function removeBasePath($url) {
  ...
  static function removeImage(&$html, $url) {
    $url = str_replace('/', '\/', $url);
    $url = str_replace('.', '\.', $url);
    $html = Html::removeTag($html, 'img', ['src', $url]);
  }
  
  static function removeImageA(&$html, $url) {
    $url = str_replace('/', '\/', $url);
    $url = str_replace('.', '\.', $url);
    $html = preg_replace(
      '/<a[^>]*><img[^>]*src=("|\'|)'.$url.'("|\'|)[^>]*><\/a>/',
  ...
  static function moveTempFiles(&$html, $tempAttacheId, $attachId) {
