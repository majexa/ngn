...
class FriendsCommon {
  ...
  function addFriend($userId, $friendId, $accepted = 0) {
    ...
    if (!$accepted) {
      ...
      $privMsgs->sendMsg($userId, $friendId, Tt()->getTpl('privMsgs/friendAddMsg', [
  ...
  function acceptFriend($userId, $friendId) {
    ...
    $privMsgs = new PrivMsgs($userId);
    $privMsgs->parseHtml = false;
    $privMsgs->sendMsg($userId, $friendId, Tt()->getTpl('privMsgs/friendAcceptMsg', [
      'userData' => get_user_info($userId)
    ]));
  ...
  function declineFriend($userId, $friendId) {
    ...
    $t = Tt()->getTpl('privMsgs/friendDeclineMsg', [
      'userData' => get_user_info($userId)
    ]);
    ...
    $privMsgs = new PrivMsgs($userId);
    $privMsgs->parseHtml = false;
    $privMsgs->sendMsg($userId, $friendId, Tt()->getTpl('privMsgs/friendDeclineMsg', [
      'userData' => get_user_info($userId)
    ]));
  }
