...
class Ganon {
  ...
  static function fileGetDom($file, $return_root = true, $use_include_path = false, $context = null) {
    $f = file_get_contents($file, $use_include_path, $context);
    return (($f === false) ? false : self::strGetDom($f, $return_root));
  }

  static function domFormat(&$root, $options = array()) {
...
class HTML_Node {
  ...
  function getChildrenByAttribute($attribute, $value, $mode = 'equals', $compare = 'total', $recursive = true) {
    ...
    return $this->getChildrenByMatch(array(
        'attributes' => array(
          $attribute => array(
...
class HTML_Selector {
  ...
  protected function error($error) {
    $error = htmlentities(str_replace(array(
        '%tok%',
        '%pos%'
      ), array(
