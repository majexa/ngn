...
class UrlParser {
  ...
  function makeClickableLinks($str) {
    ...
    $ret = preg_replace_callback($test, [$this, 'getMatchesHTTP'], $str);
  ...
  function getMatchesHTTP($matches) {
    ...
    return '<a href="'.$matches[0].'"'.$target.'>'.str_replace('http://', '', Misc::cut($matches[0], 25)).'</a>';
  }
  ...
  function removeLinks($str) {
    ...
    return preg_replace_callback($test, $this->parseLinkMatchesFunc, $str);
  ...
  function tagExtLinks($str, $activ = true, $repl = null) {
    ...
    if ($activ == null) {
      ...
      return preg_replace($test, " \${0} <a href=\"\${3}\${4}\${5}\"
  ...
  function activateImage($str) {
    ...
    return preg_replace($test, "<img src=\"\${0}\">", $str);
