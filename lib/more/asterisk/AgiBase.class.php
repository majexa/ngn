...
class AgiBase {
  ...
  function __construct($config = NULL, $optconfig = array()) {
    ...
    if (0 and $this->config['phpagi']['error_handler'] == true) {
   ...
   * @return array, see evaluate for return information. ['result'] is 1 on sucess, 0 otherwise
   */
  function setVar($pVariable, $pValue) {
    if (is_numeric($pValue)) return $this->evaluate("SET VARIABLE $pVariable $pValue");
    else
      return $this->evaluate("SET VARIABLE {$pVariable} \"{$pValue}\"");
  }
