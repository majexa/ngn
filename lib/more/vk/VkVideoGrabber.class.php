...
class VkVideoGrabber extends VkAuth {
  ...
  function init() {
    ...
    $this->oWget = new Wget(['tempFolder' => $this->dataFolder.'/temp']);
  }
    ...
    while (($items = $this->getListPageItems($st))) {
      foreach ($items as $item) {
        ...
        if (($grabbedItem = $this->processVideoPage($item))) {
          ...
          throw new Exception('Grab item error. Item: '.getPrr($item), 1028);
        }
  ...
  function getListPageItems($st = 0) {
    if (!$this->auth()) return false;
    if (!isset($this->videoListUrl))
      throw new Exception('$this->videoListUrl not defined', 1029);
    output("Parsing page st=$st");
    $url = $this->videoListUrl.($st ? '&st='.$st : '');
    output('Get page from "'.$url.'"');
    $c = $this->oCurl->get($url);
    if (!$c) throw new Exception("Content by url '$url' is empty", 1030);
    
    // parse links
    preg_match_all('/href="(video-[^"]+)"/', $c, $m);
    if (empty($m[1])) {
      throw new Exception('Video links not found. Maby not log in or too fast requests?', 311);
  ...
  function getTotalCount() {
    ...
    if (!($c = $this->oCurl->get($this->videoListUrl)))
      throw new Exception('no content by url "'.$this->videoListUrl.'"', 1031);
    preg_match('/<b>(\d+) видеозапис/s', $c, $m);
    if (empty($m[1])) {
      throw new Exception("Wrong video list page by url '{$this->videoListUrl}'", 1032);
    }
   ...
   */  
  function processVideoPage(array $item, $reflashData = false) {
    if (!$this->auth()) return false;
    //die2($item['link']);
    if (empty($item['link']))
      throw new Exception("\$item['link'] is empty", 1033);
    $url = $this->baseUrl.'/al_video.php?act=show&al=1&video='.str_replace('video', '', $item['link']);
    
    if (!($c = $this->oCurl->get($url)))
      throw new Exception('no content', 1034);
    ...
    if (!preg_match('/var vars = ({.*})/', $c, $m)) {
      ...
      if (!preg_match('/var vars = ({.*})/', $c, $m)) {
        ...
        if (!preg_match('/var vars = ({.*})/', $c, $m)) {
          ...
          throw new Exception('String "var vars = {..." not found. Contents saved to log "url"', 1035);
        }
      }
    }
        ...
        */
        ...
        if (!$this->forceDownload) {
          ...
          //if (filesize($videoFile) < 350000) {
            //unlink($videoFile);
            //throw new Exception('Too small video file (< 350 Kb)', 1037);
          //}
        }
      }
    } else {
