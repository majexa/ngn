...
const loginKeyword = 'выйти';
  ...
  function __construct($email, $pass, $dataFolder = null) {
  ...
  function auth($required = true) {
    ...
    if ($required and !$this->authorized)
      throw new Exception('Vk authorization failed', 1022);
    return $this;
  }
  ...
  protected function authByPrevSessionId() {
    ...
    if (strstr($c, self::loginKeyword)) {
      ...
      unlink($file);
      throw new Exception('Vk session failed', 1023); 
    }
  }
  ...
  protected function authByEmailAndPass() {
    ...
    $c = $this->curl->post('http://login.vk.com?act=login', [
      'al_test' => '1',
      'email' => $this->email,
    ...
    if (!$c) {
      throw new Exception('Vk login page load error', 1026);
    }
    if (strstr($c, 'либо пароль неверный')) {
      $this->saveOutput($c);
      throw new Exception('Vk auth error. Email or Password', 1025);
    }
    if (strstr($c, 'перезагружаем')) {
      throw new Exception('Vk server reload');
    }
    //$r = HttpResult::parseWithHeaders($c);
    if (!preg_match("/remixsid=([^;]+);/s", $c, $m)) {
      file_put_contents($this->dataFolder.'/error_result.html', $c);
      throw new Exception('Vk parse session ID error', 1024);
    }
    $sessionId = $m[1];
    ...
    if (!strstr($c, self::loginKeyword)) {
      $this->saveOutput($c);
      throw new Exception('Vk auth by email and password error', 1027);
    }
