...
class VkMsgs extends VkAuthBase {
  ...
  function send(array $data) {
    ...
    if (strstr($c, 'Вы попытались загрузить более')) {
      ...
      throw new Exception('Необходимо подождать 15 минут до следующей отправки сообщения', 324234);
      return;
    }
    file_put_contents($this->auth->userDataFolder.'/send_res_'.$data['to_id'].'_'.date('H-i-s'), $c);
    if (!strstr($c, '200 OK'))
      throw new Exception("=( {$data['to_id']}", 123);
  }
  
  function getLastSentUserId() {
