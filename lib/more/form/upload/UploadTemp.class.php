...
class UploadTemp extends Options2 {
  ...
  function getFiles() {
    ...
    $files = [];
    $data = db()->query('SELECT * FROM upload_temp WHERE formId=? AND tempId=?',
      $this->formId, $this->tempId);
    foreach ($data as $v) {
      ...
      if ($v['multiple']) {
      	...
      	BracketName::setValue($files, $v['fieldName']."[tmp_name][$i]",
      	  $this->tempFolder.'/'.$v['fileName']);
      	BracketName::setValue($files, $v['fileName']."[size][$i]",
  ...
  function deleteFile($name) {
    $fileName = db()->selectCell(
      'SELECT fileName FROM upload_temp WHERE tempId=? AND formId=? AND name=?', 
      $this->tempId, $this->formId, $name);
  ...
  static function deleteOld() {
    ...
    foreach (Dir::getFilesR(TEMP_PATH.'/upload') as $file)
      if (filemtime($file) < time()-$timeToKeepFiles) File::delete($file);
    Dir::removeEmpties(TEMP_PATH.'/upload');
    db()->query('DELETE FROM upload_temp WHERE dateCreate < ?', dbCurTime(time()-$timeToKeepFiles));
    foreach (db()->select('SELECT * FROM upload_temp') as $v)
      if (!file_exists(TEMP_PATH.'/upload/'.$v['tempId'].'/'.$v['fileName']))
        db()->query('DELETE FROM upload_temp WHERE tempId=? AND fileName=?', $v['tempId'], $v['fileName']);
  }
