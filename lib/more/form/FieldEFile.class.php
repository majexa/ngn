...
class FieldEFile extends FieldEFileBase {
  ...
  function defineOptions() {
    ...
    $this->options['currentFileTitle'] = 'Текущий файл';
  }

  protected function init() {
    parent::init();
    if ($this->oForm->fromRequest) {
      $files = isset($this->oForm->options['files']) ? $this->oForm->options['files'] : $this->oForm->req->files;
    ...
    if ($uploadedFileValue !== null /* and empty($uploadedFileValue['error'])*/) {
      empty($this->options['multiple']) ?
        $this->process2Single($uploadedFileValue) :
        $this->process2Multiple($uploadedFileValue);
    }
  }
  ...
  protected function htmlNav() {
    return (empty($this->options['value']) ? '' : '<div class="iconsSet fileNav">'.'<a href="'.$this->getCurrentValue().'" class="'.$this->options['currentFileClass'].'" target="_blank"><i></i> '.$this->options['currentFileTitle'].'</a>'.((!empty($this->oForm->options['deleteFileUrl']) and empty($this->options['required'])) ? '<a href="'.$this->oForm->options['deleteFileUrl'].'&fieldName='.$this->options['name'].'" class="delete confirm" title="Удалить"><i></i></a>' : '').'</div>');
  }
