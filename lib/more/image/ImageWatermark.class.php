...
class ImageWatermark {
  ...
  function make($imagePath) {
    if (!file_exists($this->watermarkImagePath))
      throw new Exception('Watermark image "'.$this->watermarkImagePath.'" does not exists');
    ...
    elseif ($watermarkInfo['mime'] == 'image/wbmp') $srcImg = imagecreatefromwbmp($watermarkPath);
      ...
      throw new Exception('Unexpected mime type of watermark image');
    ...
    elseif ($imageInfo['mime'] == 'image/wbmp') $dstImg = imagecreatefromwbmp($imagePath);
      ...
      throw new Exception('Unexpected mime type of source image');
    $srcW = ImageSX($srcImg);
    $srcH = ImageSY($srcImg);
    $dstW = ImageSX($dstImg);
    $dstH = ImageSY($dstImg);
