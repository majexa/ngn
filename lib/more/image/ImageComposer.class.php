...
class ImageComposer extends Image {
  ...
  function mosaic($smW, $smH, $dir, $resultPath) {
    $this->_mosaic($smW, $smH, Dir::getFilesR($dir, '*.jpg'), $resultPath);
  }
  
  function _mosaic($smW, $smH, $images, $resultPath) {
    $this->mozaicDepth = 0;
    $this->mosaicR($smW, $smH, $images, $resultPath);
  }
  
  function mosaicR($smW, $smH, $_images, $resultPath) {
    if (empty($_images))
      throw new Exception('$_images is empty');
    //die2(array($this->mozaicW, $this->mozaicH, $smW, $smH));
    $this->mozaicDepth++;
    if ($this->mozaicDepth == $this->mozaicMaxDepth) die2('Max depth!');
    $maxInRow = round($this->mozaicW / $smW);
    $maxInCol = round($this->mozaicH / $smH);
    if ($maxInRow == 0) throw new Exception('$maxInRow = 0');
    if ($maxInCol == 0) throw new Exception('$maxInCol = 0. $this->mozaicH='.$this->mozaicH.', $smH='.$smH);
  ...
  function compose($w, $h, $images, $resultPath, $resize = true) {
    if (count($images) < $this->minImagesInComposition)
      throw new Exception("Minimal images count in composition is {$this->minImagesInComposition}, total count is ".count($images));
    
    if (count($images) > $this->maxImagesInComposition)
      throw new Exception("Maximal images count in composition is {$this->maxImagesInComposition}, total count is ".count($images));
      ...
      for ($i=0; $i<count($images); $i++) {
        if (is_file($images[$i])) {
          if (!($sm[$i] = imageCreateFromJpeg($images[$i]))) {
            throw new Exception('Create JPEG from "'.$images[$i].'" error');
          }
        }
        else throw new Exception("File '{$images[$i]}' not exists");
      }
    }
