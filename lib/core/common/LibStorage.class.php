...
class LibStorage {
  ...
  private $storedClasses = [];

  private $zendFramworkPathtypeLibs = ['Zend', 'Dklab', 'DbSimple'];

  private $nonStrictIncludeModePathtypeLibs = ['Cache'];
  ...
  function __construct() {
    ...
    $this->includePaths = array_map(['Misc', 'clearLastSlash'], $this->includePaths);
  }
  ...
  private function fileExists($filepath) {
    if (file_exists($filepath)) return $filepath;
      ...
      foreach ($this->includePaths as $includePath) {
        if (file_exists($includePath.'/'.$filepath)) return $includePath.'/'.$filepath;
      }
    }
    return false;
  }
  ...
  function addFile($filepath) {
    if (!($_filepath = $this->fileExists($filepath))) throw new Exception("File '$filepath' not exists");
    $filepath = $_filepath;
  ...
  static function stripPhpTag($c, $filepath = false) {
    // Убираем PHP-тэг из начала файла
    $c = preg_replace('/^<\?php/m', ($filepath ? "\n// -- File: $filepath" : ''), $c);
    $c = preg_replace('/^<\?/m', ($filepath ? "\n// -- File: $filepath" : ''), $c);
    // Убираем PHP-тэг из конца файла
    return preg_replace('/\?>$/m', '', $c);
  }

  static function removeByKeyword($keyword, $code, $stripBrBefore = true) {
    return preg_replace('/'.($stripBrBefore ? "\n\n" : '').'^.*\@'.$keyword.'.*$/m', '', $code);
  }
  ...
  private function prepareStoredClasses(&$code) {
    ...
    if (preg_match_all('/^\s*class\s*([a-zA-Z_]+).*$/m', $code, $m)) foreach ($m[1] as $class) {
  ...
  private function prepareCodeIncludes(&$code, $file = null) {
    if (preg_match_all('/^\s*require_once'.'\s+(?=\(|)([a-zA-Z0-9\/. _\'"]*)(?=\)|)\s*;\s*$/m', $code, $m)) {
    ...
    // Убираем все инклюды
    $code = preg_replace('/^\s*require_once\s+([()a-zA-Z0-9\/. _\'"]*)(?=\)|)\s*;\s*$/m', // ^^
      '', $code);
  }

  private function prepareCodeLibs(&$code, $file = null, $prepareClasses = false) {
    if (preg_match('/Lib::required\("([^"]*)"\);/', $code)) throw new Exception('Lib::required() must contain only single quotes');
    ...
    foreach ($m[1] as $path) {
      ...
      $path = str_replace('.', '\\.', $path);
      $code2 = preg_replace('/Lib::required\([\'"]'.$path.'[\'"]\);\s*\n/', '', $code);
    }
  }
  ...
  private function prepareCodeLib($path, $file = null) {
    ...
    if (!$filepath) {
      ...
      // Иначе ошибка!
      else throw new Exception("Path '$path' not found".($file ? " in file '$file'" : ''));
    }
    //"\nCode:\n##############################################################\n$code");
  ...
  private function isKeyword($class) {
    return in_array($class, ['self', 'parent', 'static']);
  }
  ...
  private function prepareCodeClasses($code, $file = null) {
    ...
    foreach ($classes as $class) {
      ...
      //print "************* $class\n";
      $this->prepareCodeLib($this->transformZendFramewrokClass($class), $file);
    }
  }
  ...
  private function transformZendFramewrokClass($class) {
    $libName = preg_replace('/([a-zA-Z][a-zA-Z0-9]*)_([a-zA-Z][a-zA-Z0-9_]*)/', '$1', $class);
    if (in_array($libName, $this->zendFramworkPathtypeLibs)) $class = str_replace('_', '/', $class.'.php');
    return $class;
  }
  ...
  */
  ...
  static function buildPackage($ngnPath, $name) {
    output("Build package '$name'");
    $c = "<?php\n\n";
    $r = [];
    foreach (glob($ngnPath.'/lib/more/'.$name.'/*.class.php') as $file) {
      $r[] = [
        'parentsCnt' => count(ClassCore::getParents(str_replace('.class.php', '', basename($file)))),
        'file'       => $file
      ];
    }
    foreach (Arr::sortByOrderKey($r, 'parentCnt') as $v) {
