...
class Url {
  ...
  private static function init($url) {
    if (self::$cache and isset(self::$headers[$url])) return;
    $urlp = parse_url($url);
    $urlp['port'] = isset($urlp['port']) ? $urlp['port'] : 80;
    if (!$socket = socket_create(AF_INET, SOCK_STREAM, SOL_TCP)) return false;
    if (!$result = socket_connect($socket, $urlp['host'], $urlp['port'])) {
      return false;
    }
    // ------------ Header ------------
    if (isset($urlp['path'])) $link = isset($urlp['query']) ? $urlp['path'].'?'.$urlp['query'] : $urlp['path'];
    else $link = '/';
    ...
    while (($o = @socket_read($socket, 2048)) !== false) $out .= $o;
    if (!$out) {
      self::$contents[$url] = '';
      return;
    }
   ...
   * @return  bool
   */
  static function touch($url, $outputResultHtml = false) {
    ...
    if ($code[0] == 4) {
      output("Touching url '$url' (code: $code).\n------ Page Text ------\n$text"); //
      return false;
    }
