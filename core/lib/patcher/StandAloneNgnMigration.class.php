...
class StandAloneNgnMigration {
   ...
   */
  static function migrate($webrootFolder, $fromNgnFolder, $toNgnFolder) {
    if (!file_exists($fromNgnFolder))
      throw new Exception("Can not migrate NGN. 'From' folder '$fromNgnFolder' does not exists");
  ...
  static function getFuncName($buildN, $filepath) {
   ...
   * @return  array
   */
  static function getActualPatches($curBuild, $ngnFolder) {
    $patches = [];
   ...
   * @param integer Номер сборки
   */
  static function buildPatches($ngnFolder, $buildN) {
