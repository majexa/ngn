...
class Zip {
   ...
   */
  static function file($archive, $file, $localpath = '') {
   ...
   */
  static function dir($archive, $path, $localpath = null) {
    self::getObj()->dir($archive, $path, $localpath);
  }
  
  static function extract($from, $to, $strict = true) {
    if (!file_exists($from)) throw new Exception("File '$from' does not exists");
    return self::getObj()->extract($from, $to, $strict);
  }
  
  static function lst($archive) {
    return self::getObj()->lst($archive);
  }
  
  static function archive($tempFolder, $what, $archiveFilename, $whatRenamed = null) {
  ...
  static function add($archive, $what) {
