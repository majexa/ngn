...
doExec('Extract gramtab', BIN_DIR . '/extract_gramtab.php', [$morph_data_file, $out_dir]);
doExec('Extract graminfo header', BIN_DIR . '/extract_graminfo_header.php', [$morph_data_file, $out_dir]);
doExec('Create ancodes cache', BIN_DIR . '/extract_ancodes.php', [$morph_data_file, $out_dir]);

if(@$argv[5]) {
    if(false !== ($language = locale_to_dialing($locale))) {
        doExec('Create dialing ancodes map', BIN_DIR . '/extract_ancodes_map.php', [$morph_data_file, $language, $out_dir]);
    } else {
