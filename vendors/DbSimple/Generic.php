...
class DbSimple_Generic {
  ...
  function& connect($dsn) {
    ...
    if (@fopen('Cache/Lite.php', 'r', true)) {
      $tmp_dirs = [
        ini_get('session.save_path'), getenv("TEMP"), getenv("TMP"), getenv("TMPDIR"), '/tmp'
      ];
...
class DbSimple_Generic_Database extends DbSimple_Generic_LastError {
  ...
  protected function _query($query, &$total) {
    ...
    if (!empty($this->attributes['CACHE']) && $is_cacher_callable) {
      ...
      if (!empty($cache_params)) {
        ...
        $uniq_key = call_user_func_array([&$this, 'select'], $dummy);
    ...
    if (is_array($result) && $total) {
      ...
      $total = call_user_func_array([&$this, 'selectCell'], $query);
    }
    if (!$this->allowEmptySelect) {
      $this->allowEmptySelect = true;
      if (empty($result)) throw new NotFoundException("Empty query: ".$query[0]);
    }
    return $result;
  }

  protected $allowEmptySelect = true;

  function notEmptySelect() {
    $this->allowEmptySelect = false;
    return $this;
  }
  ...
  function _expandPlaceholdersFlow($query) {
    ...
    $query = preg_replace_callback($re, [&$this, '_expandPlaceholdersCallback'], $query);
    return $query;
  }
  ...
  function _shrinkLastArrayDimensionCallback(&$v) {
    ...
    if (!is_array($firstCell = current($v))) {
      ...
      array_walk($v, [&$this, '_shrinkLastArrayDimensionCallback']);
    }
  }
  ...
  function _cache($hash, $result = null) {
    if (is_callable($this->_cacher)) {
      ...
      if (null === $result) return $this->_cacher->get($hash);
      else
  ...
  var $_logger = null;
  var $_cacher = null;
  var $_placeholderArgs, $_placeholderNativeArgs, $_placeholderCache = [];
...
class DbSimple_Generic_LastError {
  ...
  function findLibraryCaller() {
    $caller = call_user_func([&$this, 'debug_backtrace_smart'], $this->ignoresInTraceRe, true);
    return $caller;
  }
