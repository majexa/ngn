Ngn.Frm.Mask.Fixed = new Class({
  Extends: Ngn.Frm.Mask,

  options: {
    autoSetSize: false,
    placeholder: '_',
    removeIfInvalid: false, // removes the value onblur if the input is not valid
    removeInvalidTrailingChars: false
  },
  ...
  initialize: function(options) {
    ...
    this.maskMold = this.options.mask.replace(Ngn.Frm.Mask.rulesRegex, this.options.placeholder).split('');
  ...
  keypress: function(e, o) {
    ...
    var c = String.fromCharCode(e.code), maskArray = this.maskArray, start, i, returnFromTestEntry;
    ...
    if (!o.isSelection) {
      ...
      var rstart = o.range.start, rend = o.range.end, end;
  ...
  paste: function(e, o) {
    ...
    this.element.set('value', this.maskMold.join('')).setCaretPosition(retApply.rangeStart);
    return true;
  },

  getLastValidIndex: function(elementValue) {
    var lastValidIndex = elementValue.length, placeholder = this.options.placeholder, i = elementValue.length - 1, cont;
    while (i >= 0) {
      cont = false;
  ...
  testEvents: function(index, _char, code, isRemoveKey) {
    var maskArray = this.maskArray, rule = Ngn.Frm.Mask.rules[maskArray[index]], returnFromTestEntry;
    if (!isRemoveKey) {
  ...
  createUnmaskRegex: function() {
    var fixedCharsArray = [].combine(this.options.mask.replace(Ngn.Frm.Mask.rulesRegex, '').split(''));
  ...
  testEntry: function(str, index, _char) {
    var maskArray = this.maskArray, rule = Ngn.Frm.Mask.rules[maskArray[index]], ret = (rule && rule.regex.test(_char));
    return (rule.check && ret) ? rule.check(str, index, _char) : ret;
  },

  applyMask: function(str, newRangeStart) {
    var strArray = str.split(''), maskArray = this.maskArray, maskMold = this.maskMold, rules = Ngn.Frm.Mask.rules, eli = 0, returnFromTestEntry;
