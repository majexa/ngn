...
Ngn.TextboxList = new Class({
  ...
  add: function(plain, id, html, afterEl, bitOpts) {
    var box = this.create('box', [id, plain, html], bitOpts);
    if (box) {
      if (!afterEl) {
  ...
  onAdd: function(bit) {
    if (bit.value && bit.value[1] == 'aa') throw new Error('!');
    return;
...
Ngn.TextboxListBit = new Class({
  ...
  initialize: function(value, textboxlist, options) {
    ...
    this.bit = new Element('li.' + this.prefix + '.' + this.typeprefix, this.options.properties).store('textboxlist:bit', this);
    this.bit.addEvents({
      mouseenter: function() {
...
Ngn.TextboxListBit.Editable = new Class({
  ...
  initialize: function(value, textboxlist, options) {
    ...
    this.element.addEvents({
      ...
      blur: function() {
        self.blur(true);
        if (self.options.addOnBlur && !textboxlist.options.onlyFromValues) {
          self.toBox();
        }
      }
    });
    if (this.options.addKeys || this.options.stopEnter) {
      ...
      this.element.addEvent('keydown', function(ev) {
        ...
        if (self.options.stopEnter && ev.key === 'enter') {
          ...
          ev.stop();
          if (!textboxlist.options.onlyFromValues) self.toBox();
        }
      });
    }
  },
  ...
  (function() {
    ...
    GrowingInput = new Class({
      ...
      initialize: function(element, options) {
        ...
        ['font-size', 'font-family', 'padding-left', 'padding-top', 'padding-bottom', 'padding-right', 'border-left', 'border-right', 'border-top', 'border-bottom', 'word-spacing', 'letter-spacing', 'text-indent', 'text-transform'].each(function(property) {
