...
Ngn.Dialog = new Class({
  ...
  initialize: function(_opts) {
    ...
    if (this.options.id == 'dlg') this.options.id = 'dlg' + Ngn.randString(5);
...
Ngn.Dialog.Confirm.Mem = new Class({
  ...
  initialize: function(_opts) {
    this.setOptions(_opts);
    if (Ngn.storage.get(this.options.id + 'confirmMem6')) {
  ...
  buildMessage: function(_msg) {
    eMessageCont = new Element('div');
    new Element('div', {'html': '<h3 style="margin-top:0px">' + _msg + '</h3>'}).inject(eMessageCont);
    Elements.from('<span class="checkbox"><input type="checkbox" id="confirmMem' + this.options.id + '" class="confirmMem" /><label for="confirmMem' + this.options.id + '">Больше не спрашивать по этому поводу</label></span>')[0].inject(eMessageCont);
    this.eMemCheckbox = eMessageCont.getElement('.confirmMem');
    return eMessageCont;
  },

  finishClose: function() {
    if (this.isOkClose) Ngn.storage.set(this.options.id + 'confirmMem6', this.eMemCheckbox.get('checked'));
    this.parent();
  }
