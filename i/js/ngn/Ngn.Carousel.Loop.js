  ...
  initialize: function(element, options) {
    ...
    if (this.options.debugNumbers) {
      ...
      for (var i=0; i<this.elements.length; i++) {
        ...
        Elements.from('<div style="position:absolute; background:#FF0000; padding: 2px 5px; top:15px; left:15px; color:#FFFFFF">'+(i)+'</div>')[0].inject(this.elements[i]);
        Elements.from('<div style="position:absolute; background:#555555; padding: 2px 5px; top:40px; left:15px; color:#FFFFFF">'+(this.elements[i].retrieve('initIndex'))+'</div>')[0].inject(this.elements[i]);
      }
    }
